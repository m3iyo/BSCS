...
void leaderboards(); //DISPLAYS RANKING OF THE STUDENTS, BY AVERAGE OR IN INDIVIDUAL SUBJECTS
...
void adviser_login()
{
	...
	ofstream admin1;
	ifstream admin;
	admin.open("admin_credentials.txt");
	if (!admin)
	{
		cout << "\n\n\n\n\n\n\n\t\t\t  System Error: No Account/s Found\n\n\n";
		system("pause");
		system("cls");
		...
		if (choice == 'Y')
		{
			...
			admin1.open("admin_credentials.txt");
			admin1 << stored_username;
			admin1 << std::endl;
			admin1 << stored_password;
		}
		else
		{
			cout << "\nCannot Access Page without an Administrative Account.. Please try again.";
			exit(0);
		}
	...
	{ 
		...
		admin >> stored_username;
		if(admin.eof())
		{ 
			cout<<"\n\n\n\t\t\tUsername does not exist. \n\t\t\tContact administrator if this is a problem"<<endl;
			Sleep(3000);
			main_menu();
    	}
	}
